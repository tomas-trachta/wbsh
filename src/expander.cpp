...
namespace wbsh {
	...
	std::string Expander::expandParam(const std::string& body, bool /*quoted_ctx*/) {
		...
		if (op == '-' || op == '+' || op == '=' || op == '?') {
			return evalParamDefaultOp(op, colon, name, namedValue(),
			                          body.substr(i + 1));
		}

		// Substring / array-slice: ${name:off[:len]}
		if (op == ':') {
			if (has_subscript && (subscript == "@" || subscript == "*"))
				return evalArraySlice(name, body.substr(i + 1));
			return substringExpand(namedValue(), body.substr(i + 1));
		}

		// Prefix / suffix strip.
		if (op == '#') {
			const bool greedy = (i + 1 < body.size() && body[i + 1] == '#');
			const std::string pat = body.substr(greedy ? i + 2 : i + 1);
			return stripPrefix(namedValue(), expandHeredoc(pat, false), greedy);
		}
		if (op == '%') {
			const bool greedy = (i + 1 < body.size() && body[i + 1] == '%');
			const std::string pat = body.substr(greedy ? i + 2 : i + 1);
			return stripSuffix(namedValue(), expandHeredoc(pat, false), greedy);
		}

		// Pattern replace.
		if (op == '/') {
			const bool all = (i + 1 < body.size() && body[i + 1] == '/');
			return evalParamReplace(namedValue(),
			                        body.substr(all ? i + 2 : i + 1), all);
		}
